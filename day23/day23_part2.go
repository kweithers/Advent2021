...
var starting_state = State{hallway: Hallway{46, 46, 46, 46, 46, 46, 46, 46, 46, 46, 46}, rooms: [4]Room{{68, 68, 68, 68}, {65, 67, 66, 67}, {67, 66, 65, 66}, {65, 65, 67, 66}}, cost: 0}
var goal_state = State{hallway: Hallway{46, 46, 46, 46, 46, 46, 46, 46, 46, 46, 46}, rooms: [4]Room{{65, 65, 65, 65}, {66, 66, 66, 66}, {67, 67, 67, 67}, {68, 68, 68, 68}}, cost: 0}

// var starting_state = State{hallway: Hallway{46, 46, 46, 46, 46, 46, 46, 46, 46, 46, 46}, rooms: [4]Room{{66, 68, 68, 65}, {67, 67, 66, 68}, {66, 66, 65, 67}, {68, 65, 67, 65}}, cost: 0}
	...
	for room_index, room := range st.rooms {
		for room_depth, pod := range room {
			...
			if room_depth == 1 && room[0] != 46 {
				continue
			} else if room_depth == 2 && (room[0] != 46 || room[1] != 46) {
				continue
			} else if room_depth == 3 && (room[0] != 46 || room[1] != 46 || room[2] != 46) {
				continue
			}
...
Hallway_Walk:
	for hallway_index, pod := range st.hallway {
		...
		if !(st.rooms[homes[pod]] == Room{46, 46, 46, 46} || st.rooms[homes[pod]] == Room{46, 46, 46, pod} || st.rooms[homes[pod]] == Room{46, 46, pod, pod} || st.rooms[homes[pod]] == Room{46, pod, pod, pod}) {
			continue
		}
		...
		correct_depth := 3
		if st.rooms[homes[pod]][3] == pod {
			correct_depth = 2
		}
		if st.rooms[homes[pod]][2] == pod {
			correct_depth = 1
		}
